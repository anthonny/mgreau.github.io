...
== W3C : WebSocket API (HTML5)

La {link-w3c-api}[spécification W3C] est en cours de finalisation. Elle définit les méthodes Javascript suivantes :
...
La transformation d'un POJO vers un WebSocket Server Endpoint est très simple, puisqu'il suffit d'annoter la classe avec *@ServerEndpoint* et une méthode avec *@OnMessage* :
====
[source, java]
----
...
----
<1> Il suffit d'annoter la classe avec l'annotation @ServerEndpoint et de préciser l'URI d'accès à ce Endpoint
<2> la méthode handleMessage va traiter les messages reçus et renvoyer une chaîne de caractères
====
