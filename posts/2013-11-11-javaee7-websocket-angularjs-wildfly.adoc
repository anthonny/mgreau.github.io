...
v1.0, November 11, 2013: First English version
:toc:
...
:mdash: &#8212;
:language: asciidoc
:link-demo: http://wildfly-mgreau.rhcloud.com/usopen/live.html
...
:link-wildfly: http://wildfly.org
:link-ng-directives-en: http://suhairhassan.com/2013/10/21/refactoring-to-angularjs-directive.html#.Un1t12S1Xv2
:link-ng-directives-fr: http://www.frangular.com/2013/10/scope-isole-dans-les-directives.html
:link-ng-ws: http://clintberry.com/2013/angular-js-websocket-service/

[NOTE]
.Overview
====
This blog post describes the *{link-jsr356}[Java API for WebSocket Protocol (JSR 356)]* (which is one of four newest JSRs for the *{link-javaee7}[Java EE 7]* platform) and provides a concrete application deployed on {link-wildfly}[WildFly 8] and {link-demo}[available online on OpenShift].

* {link-pdf-en}[Download an English PDF version]
* [FR] La version française ({link-html-fr}[HTML] ou {link-pdf-fr}[PDF]) de ce post est basée uniquement sur la démonstration avec l'API Javascript sans AngularJS.
====
...
image::posts/javaee7_intro.png[Java EE 7 goals]
...
image::posts/websocket_wildfly_angularjs_tennis.png[Implementation of WebSocket (Java API et Javascript API), link="{link-demo}"]
...
image::posts/WebSocket_Protocol.png[Diagram which explain how does the WebSocket protocol work,550]
...
+ src/main/java
   |+ com.mgreau.wildfly.websocket
      ...
      |+ rest       // <4>
         ...
         |- TournamentREST.java
      |- MatchEndpoint.java    // <5>
      |- StarterService.java   // <6>
      |- TennisMatch.java      // <7>
+ src/main/resources
+ scr/main/webapp
   |+ css
...
<4> REST endpoint to list all tournament's matches
<5> The application WebSocket Server Endpoint ('peer server')
<6> EJB @Startup in order to initialize this application at deployment time
<7> POJO to handle informations about the match
<8> AngularJS files to handle several matches (live.html) with REST and WebSocket calls
<9> File containing the implementation of Javascript API for WebSocket protocol to handle the client side of the communication for the simple case (index.html)
====
...
As we saw on the beginning of this post, the version to handle severals matches is developed with AngularJS on the client side.
So there are 4 JS files which contains : 
...
[NOTE]
====
'This post is not a tutorial about AngularJS since it's my first sample with this framework. But it was a quick solution to handle several matches on the client side. You can see the 3 blogs posts followed to better understand the JS code : {link-ng-directives-fr}[directives in French] and {link-ng-directives-en}[Refactoring to AngularJS directives] and {link-ng-ws}[AngularJS WebSocket Service example]'
====
...
WildFly 8 uses its new +*Web Server called http://undertow.io[Undertow]*+ which replaces Tomcat.

[TIP]
====
'I didn't test this application on Glassfish 4, but since I'm using only Java EE 7 API dependencies, it could work with the same code without problem. Let me know if you have '
====
...
[NOTE]
====
'Values between ()* represents the same tests on my laptop (MacBook Pro 2013 i5 - 8Go - 128SSD) with WildFly 8.0.0-beta1 with default configuration.'
====
